...
class EkspertnaOsobaCtl implements Controller
{
    ...
    public function displayDodavanjeJavnogAlata()
    {
        ...
        switch (get("msg")) {
            case 1:
                ...
                break;
            case 2:
                $error = "Skraćeni naziv mora se sastojati od alfanumeričkih znakova bez razmaka!";
                break;
            case 3:
                $error = "Upisani skraćeni naziv već postoji!";
                break;
            default:
                break;
        }
    ...
    public function dodajJavniAlat()
    {
        ...
        if (post("naziv") === false || post("skraceniNaziv") === false || post("inacica") === false || post("cijena") === false || post("link") === false) {
        ...
        if ($alat->loadSkraceniNaziv(post("skraceniNaziv")) == true) {
            preusmjeri(\route\Route::get('d3')->generate(array(
                "controller" => "ekspertnaOsobaCtl",
                "action" => "displayDodavanjeJavnogAlata"
            )) . "?msg=3");
        ...
        $akcijeSustava->zabiljeziNovuAkciju($_SESSION['auth'], date("Y-m-d H:i:s"), "Unosi javni alat - idAlata je " . $alat->getPrimaryKey());
    ...
    public function displayPregledJavnihAlata()
    {
        ...
        switch (get("msg")) {
            ...
            case 4:
                ...
                break;
            case 5:
                $error = "Već postoji alat s takvim skraćenim nazivom!";
                break;
            default:
                break;
        }
    ...
    public function updateJavniAlat()
    {
        if (post("checked") == true) {
            ...
            $alat          = new \model\DBAlat();
            $akcijeSustava = new \model\DBAkcijaSustava();
            $akcijeSustava->zabiljeziNovuAkciju($_SESSION['auth'], date("Y-m-d H:i:s"), "Brise javni alat - idAlata je " . post("id"));
            ...
            if ($alat->ispravanSkraceniNaziv(post("id"), post("skraceniNaziv")) === false) {
                preusmjeri(\route\Route::get('d3')->generate(array(
                    "controller" => "ekspertnaOsobaCtl",
                    "action" => "displayPregledJavnihAlata"
                )) . "?msg=5");
            ...
            $akcijeSustava = new \model\DBAkcijaSustava();
            $akcijeSustava->zabiljeziNovuAkciju($_SESSION['auth'], date("Y-m-d H:i:s"), "Ažurira javni alat - idAlata je " . $alat->getPrimaryKey());
    ...
    public function dodajZnanstveniSkup()
    {
        ...
        if (post("naziv") === false || post("mjesto") === false || post("drzava") === false || post("adresa") === false) {
            preusmjeri(\route\Route::get('d3')->generate(array(
                "controller" => "ekspertnaOsobaCtl",
    ...
    public function dodajPlatformu()
    {
        ...
        if (post("tip") === false || post("naziv") === false || post("skraceniNaziv") === false || post("inacica") === false || post("cijena") === false) {
            preusmjeri(\route\Route::get('d3')->generate(array(
                "controller" => "ekspertnaOsobaCtl",
    ...
    public function displayPrijedlogRada()
    {
        if (!\model\DBKorisnik::isLoggedIn() || $_SESSION['vrsta'] != 'E') {
            preusmjeri(\route\Route::get('d1')->generate());
        }
        ...
        try {
            ...
            if ($prijedlog->idSkupa != NULL) {
                $sk = new \model\DBZnanstveniSkup();
                $sk->load($prijedlog->idSkupa);
                $imeSkupa = $sk->naziv;
            }
            
            if ($prijedlog->idCasopisa != NULL) {
                $c = new \model\DBZnanstveniCasopis();
                $c->load($prijedlog->idCasopisa);
                $imeCasopisa = $c->naziv;
            }
            
            echo new \view\Main(array(
                "body" => new \view\PrijedlogRada(array(
                    "prijedlog" => $prijedlog,
                    "imeSkupa" => $imeSkupa,
                    "imeCasopisa" => $imeCasopisa
                )),
                "title" => "Prijedlog znanstvenog rada"
            ));
        }
        catch (opp\model\NotFoundException $e) {
            preusmjeri(\route\Route::get('d1')->generate());
    ...
    public function obradiPrijedlogRada()
    {
        if (!\model\DBKorisnik::isLoggedIn() || $_SESSION['vrsta'] != 'E') {
            preusmjeri(\route\Route::get('d1')->generate());
        }
        ...
        try {
            $prijedlog->load(get("id"));
            //brisi privremeno pohranjeni zapis prijedloga
            $lok = $prijedlog->lokacija;
            if (false !== strpos($lok, "pdf/prijedlog")) {
                //ukloni zapis rada
                unlink($lok);
            }
            $prijedlog->delete();
    ...
    public function displayPrijedloziRadova()
    {
        if (!\model\DBKorisnik::isLoggedIn() || $_SESSION['vrsta'] != 'E') {
            preusmjeri(\route\Route::get('d1')->generate());
        }
        ...
        echo new \view\Main(array(
            "body" => new \view\PrijedloziRadova(array(
                "prijedlozi" => $pov
            )),
            "title" => "Prijedlozi rnanstvenih radova"
        ));
    }
    ...
    public function displayPDFRada()
    {
        ...
        // preko get zahtjeva mi vvrati id prijedloga
        if (get("id") === false) {
            preusmjeri(\route\Route::get('d1')->generate());
        }
        
        $rad = new \model\DBPrijedlozi();
        try {
            $rad->load(get("id"));
            
            $lok = $rad->lokacija;
            
            if (false !== strpos($lok, "http") || false !== strpos($lok, "www")) {
                // utipkan je link
